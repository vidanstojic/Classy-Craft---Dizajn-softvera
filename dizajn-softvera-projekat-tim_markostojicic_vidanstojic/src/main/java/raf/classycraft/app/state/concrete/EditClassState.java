...
public class EditClassState implements State {
    ...
    private List<Attribute>listForRemoveAttributes = new ArrayList<>();
    @Override
    public void stateMousePressed(MouseEvent e, DiagramView tempTab) {
        ...
        for(ElementPainter elementPainter : tempTab.getListOfPainters()){
            ...
            Object[] selectionValuesMode = {"Add attributes", "Rename", "Remove"};
            ...
            if (selectionMode == "Rename"){
                ...
                if (selectionRename == "Rename class name") {
                    ...
                    tempTab.repaint();
                }else if (!interclass.getAttributes().isEmpty()){
                        String inputText = JOptionPane.showInputDialog("Enter full name of attribute that exist");
                        if (inputText == null) return;
                        for (Attribute attribute : interclass.getAttributes()) {
                            if (attribute.getName().equals(inputText)) {
                                listForRemoveAttributes.add(attribute);
                                String renameText = JOptionPane.showInputDialog("Enter new");
                            }
                        }
                    }
            } else if (selectionMode == "Remove") {
                if (interclass.getAttributes().isEmpty()){
                    return;
                } else {
                    String inputText = JOptionPane.showInputDialog("Enter full name of attribute that exist");
                    if (inputText == null)return;
                    ...
                    }for (Attribute attribute : listForRemoveAttributes){
                        interclass.getAttributes().remove(attribute);
                        tempTab.repaint();
                    }
                }
            } else if (selectionMode == "Add attributes") {
